...
At this point, you expect me to say that, in these cases, we use a t-test and show you, in case you forgot, how to run a t-test. That's pretty much right (although in fact a t-test rests on a more specific notion of "same" than what's  implied here). But that's not what we're here for right now. We're here to remind ourselves of the fundamentals of statistics.
...
Now - let's look at some experiments where we supposed, under both theories, that we had data that we required to be very, very similar to this data set (similar to a bootstrap, but with a bit of smoothing).
...
More to the point: **the false positive rate of a test is only right if the assumptions about the data are right.**
